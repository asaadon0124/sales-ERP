<div class="table-responsive">
    <div class="card-header ">
        ...
        @if (!empty($get_active_shift))
            @can('اضافة فاتورة المشتريات')
                <button class="btn btn-primary" wire:click.prevent="$dispatch('purchaseOrderCreate')">اضافة</button>
            @endcan
        @endif
                        ...
                        <td>
                            @if ($item->approve == 0 && !empty($get_active_shift))
                                <div class="d-flex align-items-center">
                                    @if (!empty($item->order_detailes))
                                        @can('تعديل فاتورة المشتريات')
                                            <a class="btn btn-primary waves-effect waves-float waves-light" title="Edit"
                                                href="#"
                                                wire:click.prevent="$dispatch('purchaseOrderUpdate', {id: {{ $item->auto_serial }}})">
                                                نعديل
                                            </a>
                                        @endcan
                                    @endif
                                    ...
                                    @can('حذف فاتورة المشتريات')
                                        <a class="btn btn-danger waves-effect waves-float waves-light mr-3" href="#"
                                            data-id="{{ $item->id }}"
                                            wire:click.prevent="$dispatch('purchaseOrderDelete', {id: {{ $item->auto_serial }}})"
                                            ti`le="Delete">
                                            حذف
                                        </a>
                                    @endcan
                                    ...
                                    @if ($item->order_detailes->count() > 0)
                                        @can('اعتماد فاتورة المشتريات')
                                            <a class="btn btn-success waves-effect waves-float waves-light mr-3"
                                                href="#" data-id="{{ $item->id }}"
                                                wire:click.prevent="$dispatch('purchaseOrderApprove', {id: {{ $item->auto_serial }}})"
                                                title="Approve">
                                                اعتمد
                                            </a>
                                        @endcan
                            ...
                            @can('تفاصيل فاتورة المشتريات')
                                <a class="btn btn-warning waves-effect waves-float waves-light" title="Show" wire:navigate
                                    href="{{ route('purchaseOrders.show', $item->auto_serial) }}">
                                    المزيد
                                </a>
                            @endcan
