<div class="table-responsive" wire:ignore.self>
    ...
    @if (!empty($servant))
        <h4 class="text-center mb-4">
            الرصيد الحالي للمندوب:
            <strong class="{{ $servant->current_balance < 0 ? 'text-danger' : 'text-success' }}">
                {{ $servant->current_balance }}
            </strong>
        </h4>
    @endif
    ...
    <div class="tab-content" id="pills-tabContent">
        ...
        <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
            @if (!empty($data) && !empty($servant))
                <div class="text-center mb-3">
                    <h5 class="text-primary">كشف حساب المندوب: <strong>{{ $servant->name }}</strong></h5>
                    @if ($start_date != '')
                    ...
                    @if (!empty($data) && !empty($servant))
                        ...
                        @foreach ($data as $item)
                            <tr>
                                ...
                                <td>{{ $item->account_balance_servant_before == 0 ?  $item->account_balance_before: $item->account_balance_servant_before }}</td>
                                <td>{{ $item->cash_amount }}</td>
                                <td>{{ $item->account_balance_servant_after == 0 ?  $item->account_balance_after: $item->account_balance_servant_after }}</td>
        ...
        <div class="tab-pane fade" id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab">
            @if (!empty($data_invoices) && !empty($servant))
                <div class="text-center mb-3">
                    <h5 class="text-primary">فواتير المشتريات من المندوب: <strong>{{ $servant->name }}</strong></h5>
                    @if ($start_date != '')
                        <p class="text-muted">الفترة من <span class="text-info">{{ $start_date }}</span> إلى <span
                                class="text-info">{{ $end_date }}</span></p>
                    @else
