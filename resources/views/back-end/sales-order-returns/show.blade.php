        ...
        <div class="card-body">
            <div id="example1_wrapper" class="dataTables_wrapper dt-bootstrap4">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <table id="example1" class="table table-bordered table-striped dataTable" role="grid"
                            ...
                            @if (isset($order))
                                ...
                                {{-- servant_code اسم العميل  --}}
                                <tr role="row">
                                    <td style="width: 30%;background-color:#d6cacae6;">اسم المندةب</td>
                    ...
                    <h4 class="text-center mb-5 mt-5">اصناف فاتورة مرتجع المبيعات رقم {{ $order->auto_serial }}</h4>


                    <div class="card-header d-flex justify-content-between">
                        @if ($order->created_by == auth()->user()->id && $order->approve == 0)
                             <button class="btn btn-primary"
                            wire:click.prevent="$dispatch('orderDetailesReturnsCreate',{id: {{ $orderId }}})">اضافة صنف
                            جديد</button>
                        @endif


                        <input type="text" wire:model.live="search" class="form-control w-25" placeholder="بحث">
                         <a href="{{ route('salesOrder.index_returns') }}" class="btn btn-danger">رجوع</a>
                    </div>
                            ...
                            @if (isset($data))
                                @foreach ($data as $iteem)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($order->created_by == auth()->user()->id && $order->approve == 0)
                                                <a class="btn btn-primary waves-effect waves-float waves-light" title="Edit" href="#" wire:click.prevent="$dispatch('salesOrderDetailesReturnsUpdate', {id: {{ $iteem->id }}})">
                                                    نعديل
                                                </a>
                                            @endif

                                            <a class="btn btn-info waves-effect waves-float waves-light" title="Edit" href="#" wire:click.prevent="$dispatch('salesOrderDetailesReturnsShow', {id: {{ $iteem->id }}})">
                                                عرض
                                            </a>

                                           @if ($order->created_by == auth()->user()->id && $order->approve == 0)
                                                <a class="btn btn-danger waves-effect waves-float waves-light" title="Edit" href="#" wire:click.prevent="$dispatch('salesOrderDetailesReturnsDelete', {id: {{ $iteem->id }}})">
                                                    حذف
                                                </a>
                                            @endif
                                        </td>
