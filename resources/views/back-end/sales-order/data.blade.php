<div class="table-responsive">
    <div class="card-header ">
        ...
        @if (!empty($get_active_shift))
            @can('اضافة فاتورة المبيعات')
                <button class="btn btn-primary" wire:click.prevent="$dispatch('salesOrderCreate')">اضافة</button>
            @endcan
        @endif
    </div>
                        ...
                        <td>
                            @if (!empty($get_active_shift))
                                <div class="d-flex align-items-center">
                                    @can('تعديل فاتورة المبيعات')
                                        <a class="btn btn-primary waves-effect waves-float waves-light" title="Edit"
                                            href="#"
                                            wire:click.prevent="$dispatch('salesOrderUpdate', { id: '{{ $item->auto_serial }}' })"
>
                                            نعديل
                                        </a>
                                    @endcan

                                    {{-- @can('حذف فاتورة المبيعات')
                                        <a class="btn btn-danger waves-effect waves-float waves-light mr-3 ml-3" href="#"
                                            data-id="{{ $item->id }}"
                                            wire:click.prevent="$dispatch('salesOrderDelete', {id: {{ $item->auto_serial }}})"
                                            ti`le="Delete">
                                            حذف
                                        </a>
                                    @endcan --}}
                            ...
                            @can('تفاصيل فاتورة المبيعات')
                                <a class="btn btn-warning waves-effect waves-float waves-light" title="Show" wire:navigate
                                    href="{{ route('salesOrder.show', $item->auto_serial) }}">
                                    المزيد
                                </a>
                            @endcan
</div>
