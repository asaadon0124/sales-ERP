                ...
                <div class="row">
                    ...
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <input type="text" wire:model.live="search" class="form-control w-25" placeholder="بحث">
                         <a href="{{ route('purchaseOrders.index_returns') }}" class="btn btn-danger">رجوع</a>
                    </div>
                            ...
                            @if (isset($data))
                                @foreach ($data as $iteem)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($order->created_by == auth()->user()->id && $order->approve == 0)
                                                <a class="btn btn-primary waves-effect waves-float waves-light" title="Edit" href="#" wire:click.prevent="$dispatch('purchaseOrderDetailesReturnsUpdate', {id: {{ $iteem->id }}})">
                                                    نعديل
                                                </a>
                                            @endif

                                            <a class="btn btn-info waves-effect waves-float waves-light" title="Edit" href="#" wire:click.prevent="$dispatch('purchaseOrderDetailesReturnsShow', {id: {{ $iteem->id }}})">
                                                عرض
                                            </a>

                                           @if ($order->created_by == auth()->user()->id && $order->approve == 0)
                                                <a class="btn btn-danger waves-effect waves-float waves-light" title="Edit" href="#" wire:click.prevent="$dispatch('purchaseOrderDetailesReturnsDelete', {id: {{ $iteem->id }}})">
                                                    حذف
                                                </a>
                                            @endif
                                        </td>
