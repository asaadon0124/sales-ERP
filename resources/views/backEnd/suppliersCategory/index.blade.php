...
@section('title', 'اقسام الموردين')
@section('page_name', 'اقسام الموردين')
@section('main_page_name', 'الرئيسية')
@section('current_page_name', 'اقسام الموردين ')
...
@section('content')
    ...
    <section class="content">
        @livewire('back-end.suppliers-category.data')
        @livewire('back-end.suppliers-category.create')
        @livewire('back-end.suppliers-category.update')
        @livewire('back-end.suppliers-category.delete')
...
<script>
    ...
    {
        Livewire.on('suppliersCategoryUpdateMS', function()
        {
            toastr.success('Update Successfully', 'Updated', { timeOut: 5000 });
        });


        Livewire.on('suppliersCategoryCreateMS', function()
        {
            toastr.success('تم اضافة القسم بنجاح', 'رسالة اضافة', { timeOut: 5000 });
        });


        Livewire.on('suppliersCategoryErrorMS', function()
        {
            toastr.error('Error This Item Allready Find', 'Error Message', { timeOut: 5000 });
        });

        // Livewire.on('suppliersCategoryShow', function(data)
        // {
