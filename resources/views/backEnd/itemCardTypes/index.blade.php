...
@section('title', 'انواع حركات الصنف')
@section('page_name', 'انواع حركات الصنف')
@section('main_page_name', 'الرئيسية')
@section('current_page_name', 'انواع حركات الصنف ')
...
@section('content')
    ...
    <section class="content">
        @livewire('back-end.item-card-movement-type.data')
        @livewire('back-end.item-card-movement-type.create')
        @livewire('back-end.item-card-movement-type.update')
        @livewire('back-end.item-card-movement-type.delete')
...
<script>
    ...
    {
        Livewire.on('ItemCardMovementTypeUpdateMS', function()
        {
            toastr.success('Update Successfully', 'Updated', { timeOut: 5000 });
        });


        Livewire.on('ItemCardMovementTypeCreateMS', function()
        {
            toastr.success('تم اضافة القسم بنجاح', 'رسالة اضافة', { timeOut: 5000 });
        });


        Livewire.on('ItemCardMovementTypeErrorMS', function()
        {
            toastr.error('Error This Item Allready Find', 'Error Message', { timeOut: 5000 });
        });

        // Livewire.on('ItemCardMovementTypeShow', function(data)
        // {
