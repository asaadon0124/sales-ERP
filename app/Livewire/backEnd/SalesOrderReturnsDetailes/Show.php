...
class Show extends Component
{
    ...
    public $total;
     ...
     protected $listeners = ['salesOrderDetailesReturnsShow'];


 public function salesOrderDetailesReturnsShow($id)
    {
        // fill $slider with the eloquent model of the same id
        $this->item                     = SalesOrderDetail::with('item','order')->where('id',$id)->first();
        $this->name                     = $this->item->item->name;
        $this->item_code                = $this->item->item_code;
        $this->order_type               = $this->item->order->order_type;
    ...
    {
        // dd($this->order_type);
        return view('back-end.sales-order-returns-detailes.show',
         [
