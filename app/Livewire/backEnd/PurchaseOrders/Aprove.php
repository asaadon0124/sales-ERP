...
class Aprove extends Component
{
    ...
    public $paid                = 0;
    public $unpaid              = 0;
    public $supplier_balance_after              = 0;
    ...
    public function submit(ActionHistoryService $action_history,AccountBalanceService $updateAccountBalance)
    {
        ...
        // 4 -- UPDATE ITEM BATCHES TABLE تعديل جدول بتشات الصنف *******************************
            ...
            foreach ($this->order_items as $order_detail)
            {
                ...
                $order_detail->qty_before_all_stores = ItemBatch::where('item_code', $order_detail->item_code)->selectRaw('SUM(qty - deduction) as total')->value('total') ?? 0;
                $order_detail->qty_before_this_store = ItemBatch::where('item_code', $order_detail->item_code)->where('store_id', $this->order->store_id)->selectRaw('SUM(qty - deduction) as total')->value('total') ?? 0;
        ...
        // 7 -- UPDATE SUPPLIERS TABLE تعديل جدول الخاص بالمورد  *******************************
            ...
            $this->order->supplier_balance_after = $updateAccountBalance->getCurrentBalance($this->order->account_number,'supplier','account_customer');
            $this->order->save();
            if ($this->paid > 0)
            {
